    ...
    for m=3:length(methods_folders)
        ...
        for f=3:length(filter_folder)
            ...
            if name_filter=="Avg"
                ...
                avg_means.avg_filter=additional_column;
                for j=1:length(heatmap_vars)
                    column_name=heatmap_vars(j);
                    title=titles(j);
                    h=heatmap(avg_means,"avg_filter","filter_size", ColorVariable=column_name,Title=title,XLabel=" ", YLabel="Filter size",FontSize=15);
                    saveas(h,strcat(heatmap_path,name_method,"_",column_name,".jpg"));
                end

                quality_column=repmat(quality(q),height(avg_means),1);
                avg_means=addvars(avg_means, quality_column);
                avg_means=removevars(avg_means,"avg_filter");
            ...
            elseif name_filter=="Gauss"
                ...
                for j=1:length(heatmap_vars)
                    column_name=heatmap_vars(j);
                    title=titles(j);
                    h=heatmap(gauss_means,"filter_size","sigma", ColorVariable=column_name, Title=title, XLabel="Filter size", YLabel="\sigma",FontSize=15);
